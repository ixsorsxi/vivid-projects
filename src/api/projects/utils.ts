...
export const handleDatabaseError = (error: PostgrestError | null): ProjectApiError => {
  ...
  } else if (error.message && error.message.includes('policy') || error.message && error.message.includes('infinite recursion')) {
    return new Error('Database access denied: You do not have permission to perform this action.') as ProjectApiError;
  } else {
